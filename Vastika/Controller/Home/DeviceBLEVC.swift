...
class DeviceBLEVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        {
            self.lblDetailsHeader.text = "Device Details"
        }
        ...
        let statusMains = self.dicrDta.object(forKey: "status_mains") as? String
            ...
            if (error == "0"){
                ...
                else if statusMains == "0" && error == "10" && statusUPS == "0"
                {
                    ...
                    self.getErrorString = "The mains Grid voltage is very high and shifted to UPS Mode; please check the Mains power"
                }else if (error == "12"){
    ...
    @IBAction func tapSwitchONOFF(_ sender: UIButton) {
        ...
        if resetSwitch == "1"
        {
            ...
            let alert = UIAlertController(title: webServices.AppName, message: "Are you sure to switch off the Load as this will switch off the load", preferredStyle: UIAlertController.Style.alert)
    ...
    override func viewWillAppear(_ animated: Bool) {
        self.timerforMode = Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(updateMode), userInfo: nil, repeats: true)
...
//                    self.outFreChart.addSubview(self.firstView)
        ...
        else if statusUPS == "1" && statusMains == "1"
        {
            ...
            else if mainsOK == "0" && chg_i == "0.00" && (commintErrorCode == "10" || commintErrorCode == "7")
            {
                if self.previousRunningMode != "3"
                {
                    for view in self.outFreChart.subviews {
...
//                    self.outFreChart.addSubview(self.firstView)
    ...
    func gettingErrorCode()
    {
        ...
        if ernp == false
        {
            ...
            if statusFrontOff == "0"
            {
                completeError = completeError + "The front switch is off, so please turn on the mains switch."
...
//
        ...
        if statusUPS == "1" && statusMains == "0"
        {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                        UIView.animate(withDuration: 1) {
                            ...
                            self.gaugeView.updateValueTo(CGFloat(35))
                        }
                    }
        ...
        else if statusUPS == "1" && statusMains == "1"
        {
            ...
            else if mainsOK == "0" && chg_i == "0.00" && (error == "10" || error == "7")
            {
                self.lblHeaderOutputFrequency.text = "Low Voltage"
                DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                            UIView.animate(withDuration: 1) {
